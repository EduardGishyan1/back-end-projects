...
@rental_router.get("/")
async def retrieve_rental_history(current_user:str = Depends(auth_utils.get_current_user),
                                  db = Depends(db_connection.get_db)):
    """Show rental history"""
    return await db.fetch(db_commands.SELECT_ALL_FROM_RENTALS,current_user)
    
@rental_router.post("/")
async def rent_movie(rental:schemas.Rental,current_user:str = Depends(auth_utils.get_current_user),
                     db = Depends(db_connection.get_db)):
    """Rent movie"""
